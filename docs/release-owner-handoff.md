...
- Confirm `README.md`, `CHANGELOG.md`, `pyproject.toml`, `src/earnings_call_risk_map/version.py`, and `docs/release-notes-v0.9.0.md` all identify `0.9.0`.
...
- Create the annotated tag only after the release owner accepts the worktree and evidence set: `git tag -a v0.9.0 -m "v0.9.0"`.
- Create the GitHub release only after the tag has been pushed and release notes have been reviewed: `gh release create v0.9.0 --title "v0.9.0" --notes-file docs/release-notes-v0.9.0.md`.
...
- `PYTHONPATH=src python -m earnings_call_risk_map version` prints exactly `0.9.0`.
...
Expected package dry-run version output: `0.9.0`.
