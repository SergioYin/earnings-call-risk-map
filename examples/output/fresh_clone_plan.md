...
Local evidence that a clean clone imports, tests, generates artifacts, and keeps local-only boundaries.
...
- Network is needed only for `git clone` and optional `pip` upgrade/install steps. Package commands themselves are local-file only.
...
- If `release-assets` reports missing files, run `PYTHONPATH=src python scripts/selfcheck.py` and review missing paths in `verification/fresh-clone/release_assets.json`.
