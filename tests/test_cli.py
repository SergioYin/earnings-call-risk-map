...
class CliTests(unittest.TestCase):
    ...
    def test_version(self):
        ...
        self.assertEqual(result.stdout.strip(), "0.9.7")
    ...
    def test_source_boundary_evidence_outputs_markdown_and_json(self):
        ...
        self.assertEqual(receipt["public_source_demo_receipt_count"], 3)
        ...
        self.assertTrue(receipt["checks"]["all_public_source_demo_receipts_present"])
        self.assertTrue(receipt["checks"]["all_public_source_demo_receipt_artifacts_exist"])
        ...
        demo_receipts = {item["fixture_slug"]: item for item in receipt["public_source_demo_receipts"]}
        self.assertEqual(
            set(demo_receipts),
            {"consumer_hardware", "semiconductor_equipment", "public_apple_static_case_study"},
        )
        apple_demo_receipt = demo_receipts["public_apple_static_case_study"]
        self.assertEqual(apple_demo_receipt["receipt_type"], "fixture_scoped_public_source_demo")
        self.assertEqual(apple_demo_receipt["fixture_path"], "examples/input/public_apple_static_case_study.json")
        self.assertEqual(apple_demo_receipt["missing_demo_artifact_count"], 0)
        self.assertTrue(apple_demo_receipt["checks"]["local_only_demo_scope"])
        self.assertTrue(apple_demo_receipt["checks"]["all_demo_artifacts_exist"])
        self.assertEqual(
            [artifact["path"] for artifact in apple_demo_receipt["demo_artifact_status"]],
            [
    ...
    def test_publication_checklist_outputs_markdown_and_json(self):
        ...
        self.assertIn("git tag -a v0.9.7", md_result.stdout)
        self.assertIn("gh release create v0.9.7", md_result.stdout)
        ...
        self.assertIn("--notes-file docs/release-notes-v0.9.7.md", release_step["commands"][0])
    ...
    def test_release_owner_handoff_outputs_markdown_and_json(self):
        ...
        self.assertIn("release-owner-compare-blockers", md_result.stdout)
        self.assertIn("gh release create", md_result.stdout)
        self.assertIn("docs/release-notes-v0.9.7.md", md_result.stdout)
        ...
        self.assertIn("examples/output/release_owner_compare_blockers.json", payload["promotion_evidence_paths"])
    ...
    def test_audit_reports_package_parity_json_and_markdown(self):
        ...
        self.assertIn("release-owner-compare-blockers", payload["commands"])
    ...
    def test_release_assets_reports_current_version_assets(self):
        ...
        self.assertIn("docs/release-notes-v0.9.7.md", payload["expected_assets"])
        ...
        self.assertIn("- [x] `docs/release-notes-v0.9.7.md`", md_result.stdout)
    ...
    def test_release_assets_reports_missing_assets(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "docs").mkdir()
            (root / "docs" / "release-notes-v0.9.7.md").write_text("release notes", encoding="utf-8")
            ...
            self.assertIn("- [x] `docs/release-notes-v0.9.7.md`", markdown)
    ...
    def test_release_notes_renders_current_audit_assets_and_changelog(self):
        ...
        self.assertIn("- Version: `0.9.7`", result.stdout)
        ...
        self.assertIn("## 0.9.7 - 2026-07-07", result.stdout)
        self.assertIn("Release-owner compare blocker checklist increment.", result.stdout)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            commands = [
                ...
                (
                    ...
                    "markdown",
                    "--output",
                    str(tmp_path / "release_owner_compare_blockers.md"),
                ),
                (
                    "release-owner-compare-blockers",
                    "--compare",
                    ...
                    "json",
                    "--output",
                    str(tmp_path / "release_owner_compare_blockers.json"),
                ),
    ...
    def test_demo_writes_static_dashboard(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("gh release create v0.9.7", publication_checklist_md)
    ...
    def test_maturity_evidence_writes_bundle(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload["command_count"], 35)
            self.assertEqual(payload["fixture_count"], 7)
            ...
            self.assertIn("examples/output/release_owner_compare_blockers.md", payload["artifact_paths"])
            self.assertIn("examples/output/release_owner_compare_blockers.json", payload["artifact_paths"])
            ...
            self.assertIn("- Commands: 35", markdown)
